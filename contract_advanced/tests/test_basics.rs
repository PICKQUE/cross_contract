...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let hello_contract_wasm = std::fs::read("./tests/external_contracts/hello-near.wasm")?;
    let hello_contract = worker.dev_deploy(&hello_contract_wasm).await?;

    let guest_book_contract_wasm = std::fs::read("./tests/external_contracts/guest-book.wasm")?;
    let guest_book_contract = worker.dev_deploy(&guest_book_contract_wasm).await?;

    let counter_contract_wasm = std::fs::read("./tests/external_contracts/counter.wasm")?;
...
async fn test_multiple_contracts(
    ...
    let _ = user
        .call(guest_contract.id(), "add_message")
